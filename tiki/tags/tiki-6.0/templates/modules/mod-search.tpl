{* $Id: mod-search.tpl 29993 2010-10-13 17:23:57Z jonnybradley $ *}
{*strip TODO*}
...
{if $module_params.tiki_search neq 'none'}
    <form id="search-module-form{$search_mod_usage_counter}" method="get" action="#"{if $module_params.use_autocomplete eq 'y'} onsubmit="return submitSearch{$search_mod_usage_counter}()"{/if}>
    	<div>
		    <input id="search_mod_input_{$search_mod_usage_counter}" name="find"{if $module_params.use_autocomplete eq 'y'} class="pagename"{/if}{if !empty($module_params.input_size)} style="width:{$module_params.input_size}em"{/if} type="text" accesskey="s" value="{$module_params.input_value}" /> 

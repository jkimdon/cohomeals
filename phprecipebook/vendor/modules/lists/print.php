...
$store_id = (isset($_REQUEST['store_id'])) ? stripslashes( $_REQUEST['store_id'] ) : 0; // the selected store layout
...
if ($store_id > 0)
{
	...
	$sql = "SELECT store_layout FROM $db_table_stores WHERE store_owner = '" . $SMObj->getUserLoginID() . "' AND store_id=$store_id";
	$rc = $DB_LINK->Execute($sql);
	DBUtils::checkResult($rc, NULL, NULL, $sql);

...
if ($mode=="add") {
	$sql = "SELECT favorite_recipe FROM $db_table_favorites WHERE favorite_owner='" . $SMObj->getUserLoginID() . "' AND favorite_recipe=$recipe_id";
	$rc = $DB_LINK->Execute( $sql );
	DBUtils::checkResult($rc,NULL,NULL,$sql);
	if ($rc->RecordCount()>0) {
		echo "<font color=red>" . $LangUI->_('Favorite is already in list!') . "</font>";
	} else {
		$sql = "INSERT INTO $db_table_favorites (favorite_owner, favorite_recipe) VALUES ('" . $SMObj->getUserLoginID() . "',$recipe_id)";
		$rc = $DB_LINK->Execute( $sql );
		DBUtils::checkResult($rc,$LangUI->_('Favorite Successfully Added'),$LangUI->_('Failed to add Favorite'),$sql);
	}
} else if ($mode=="delete") {
	$sql = "DELETE FROM $db_table_favorites WHERE favorite_owner='" . $SMObj->getUserLoginID() . "' AND favorite_recipe=$recipe_id";
	$rc = $DB_LINK->Execute( $sql );

...
<?php
$sql = "SELECT avg(rating_score) FROM $db_table_ratings WHERE rating_recipe=" . $recipe_id;
...
$sql = "SELECT review_owner, review_comments, review_date, user_name FROM $db_table_reviews 
		LEFT JOIN $db_table_users ON user_login = review_owner WHERE review_recipe=$recipe_id ORDER BY review_date";
$rc = $DB_LINK->Execute($sql);
// Error check
DBUtils::checkResult($rc, NULL, NULL, $sql);

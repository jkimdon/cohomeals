...
$base_id = isset($_REQUEST['base_id']) ? $_REQUEST['base_id'] : 0;
$ethnic_id = isset($_REQUEST['ethnic_id']) ?  $_REQUEST['ethnic_id'] : 0;
$time_id = isset($_REQUEST['time_id']) ? $_REQUEST['time_id'] : 0;
$difficult_id = isset($_REQUEST['difficult_id']) ? $_REQUEST['difficult_id'] : 0;
$cost = isset($_REQUEST['cost']) ? $_REQUEST['cost'] : 0.00;
...
<?php
	...
	if (!isset($_REQUEST["search"])) {
		...
		if ($course_id) $query .= " recipe_course=" . $course_id . " AND";
		if ($base_id) $query .= " recipe_base=" . $base_id . " AND";
		if ($ethnic_id) $query .= " recipe_ethnic=" . $ethnic_id . " AND";
		if ($time_id) $query .= " recipe_prep_time=" . $time_id . " AND";
		if ($difficult_id) $query .= " recipe_difficulty=" . $difficult_id . " AND";
		if ($cost) $query .= " recipe_cost " . $_REQUEST["cost_compare"] . " " . htmlentities($cost, ENT_QUOTES) . " AND";
		if ($owner) $query .= " recipe_owner='" . $owner . "' AND";
		if ($keywords != "") {
			$query .= " recipe_name LIKE '%". htmlentities($keywords, ENT_QUOTES) . "%' OR ";
			$query .= " recipe_directions LIKE '%".htmlentities($keywords, ENT_QUOTES) . "%' OR ";
			$query .= " recipe_source LIKE '%". htmlentities($keywords, ENT_QUOTES) . "%' OR ";
			$query .= " recipe_comments LIKE '%". htmlentities($keywords, ENT_QUOTES) . "%'";
		}

...
$total_ingredients = isset( $_REQUEST['total_ingredients'] ) ? $_REQUEST['total_ingredients'] : 0;
$total_related = isset ($_REQUEST['total_related'] ) ? $_REQUEST['total_related'] : 0;
...
if (!$SMObj->checkAccessLevel("AUTHOR")) {
	...
	$sql = "SELECT recipe_owner FROM $db_table_recipes WHERE recipe_id = $recipe_id";
...
<p>
<?php
// If there is an existing recipe list the ingredients
if ($recipe_id) {
	...
	$sql = "SELECT $db_table_ingredientmaps.*,
			...
			WHERE map_recipe = $recipe_id ORDER BY map_order";
	...
	$sql = "SELECT related_child,related_required FROM $db_table_related_recipes WHERE related_parent=" . $recipe_id;
...
?>
	...
	<th><?php echo $LangUI->_('Ingredient');?></th>
...
<?php
	...
	for ($i=0; $i < $total_ingredients; $i++) {
		if (isset($_REQUEST['refresh'])) {
			$ingredient_id= $_POST['ingredient_id_'.$i];
			$ingredient_qual= $_POST['ingredient_qualifier_'.$i];
			$ingredient_quant = $_POST['ingredient_quantity_'.$i];
			$ingredient_unit = $_POST['ingredient_unit_'.$i];
...
<br /><?php echo $LangUI->_('Number of fields to display')?>: <input type="text" name="total_ingredients" size="2" value="<?php echo $total_ingredients;?>">
...
<?php	
	...
	echo "</TABLE>\n";
	echo "<br />" . $LangUI->_('Number of fields to display') . ': <input type="text" name="total_related" size="2" value="' . $total_related . '">  ';

...
if ($mode == "add") {
	...
	$sql = "INSERT INTO $db_table_stores (store_name, store_layout, store_owner) VALUES ('$store_name', '', '$store_owner')";
	...
	while (isset($_POST[$this_store_name])) {
		...
		if (isset($_POST[$store_delete])) {
			...
			$sql = "DELETE FROM $db_table_stores WHERE store_id='" . $_POST[$this_store_id] . "' AND store_owner='" . $SMObj->getUserLoginID() . "'";
			$rc = $DB_LINK->Execute($sql);
			DBUtils::checkResult($rc, NULL, NULL, $sql);
		} else {
			// update the entry to the new value
			$sql = "UPDATE $db_table_stores SET 
							store_name='" . htmlentities($_POST[$this_store_name], ENT_QUOTES) .
					"' WHERE store_id='" . $_POST[$this_store_id] . "' AND store_owner='" . $SMObj->getUserLoginID() . "'";
			$rc = $DB_LINK->Execute($sql);
			DBUtils::checkResult($rc, NULL, NULL, $sql);
		}

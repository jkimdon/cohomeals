...
class Ingredient {
	...
	*/
	function update() {
		...
		$sql = "
			...
			SET ingredient_name='$this->name',
				ingredient_desc='$this->description',
				ingredient_location=$this->location,
				ingredient_solid='$this->solid',
				ingredient_price=$this->price,
				ingredient_unit=$this->unit,
				ingredient_system='$this->system'
	...
	*/
	function delete() {
		...
		$sql = "DELETE from $db_table_ingredients WHERE ingredient_id=" . $this->id;
		$rc = $DB_LINK->Execute($sql);
		DBUtils::checkResult($rc, NULL, NULL, $sql);
		
		$sql = "DELETE from $db_table_ingredientmaps WHERE map_ingredient=" . $this->id;
		$result = $DB_LINK->Execute($sql);
		DBUtils::checkResult($rc, NULL, NULL, $sql);
		
		$sql = "DELETE from $db_table_list_ingredients WHERE list_ing_ingredient=" . $this->id;
		$result = $DB_LINK->Execute($sql);
		DBUtils::checkResult($rc, NULL, NULL, $sql);
	}
	...
	*/
	function loadIngredient() {
		...
		if ($this->price==NULL) {
			$sql = "SELECT * FROM $db_table_ingredients WHERE ingredient_id=$this->id";
			$rc = $DB_LINK->Execute($sql);

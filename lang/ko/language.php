...
"Email" => "이메일",
"Allows an email address to be input with options of making it active" => "이메일 주소 입력을 활성화할 수 있는 선택사항과 함께 허용",
"Link Type" => "링크 유형",
...
"Location" => "위치",
"Allow a geolocation to be selected for the item and displays it on a map" => "항목에 대한 지리적 위치를 선택하고 지도에 표시하는 것을 허용합니다",
...
"Allows tags on pages and various objects to be set within the website and generate tag cloud navigation patterns." => "웹사이트 내부의 페이지와 다양한 개체에 태그를 설정 및 태크 클라우드 탐색 패턴을 허용",
"Action Log" => "작업 로그",
"Provides ability to track what users are doing and produce reports on a per-user or per-category basis." => "사용자가 하는 일을 추적 및 사용자 당 혹은 범주 방에 기반하여 보고서를 제작하는 것을 허용",
"Contribution" => "기여도",
...
"Allows the menu options that can be seen by a user to be controlled" => "사용자에게 표시되는 메뉴 선택사항을 제어하는 것을 허용함",
"Group Alert" => "그룹 알림",
"Integrator" => "통합기",
...
"Going beyond Backlinks, allows some semantic relationships to be defined between wiki pages" => "백링크 너머 이동, 위키 페이지들 간의 어떤 시맨틱 관계를 정의하는 것을 허용합니다",
"Web Services" => "웹 서비스",
...
"Minimum amount of seconds to be elapsed between source checks to prevent flooding servers" => "서버를 폭주하는 것을 방지하기 위하여 소스 체크 사이의 흐르게 될 초의 최소 양",
...
"Multi-domain" => "다중 도메인",
"Allows domain names to be mapped to perspectives and simulate multiple domains hosted on the same instance" => "도메인명을 퍼스펙티브로 매핑하고 동일한 인스턴스 상에 호스트된 멀티도메인을 시뮬레이트하는 것을 허용합니다",
...
"Mail Delivery" => "메일 발송",
"When set to Queue will store messages in the database. Requires to use the shell script ./sendmail.php to be run for actual delivery. Only works with SMTP mail" => "Queue로 설정될 때 메시지를 데이터베이스에 저장될 것입니다. 실제 전달을 위해서는 쉘스크립트가 실행되는 것이 요구됩니다. SMTP메일에서만 작옹합니다",
...
"Can subscribe many emails at once (requires tiki_p_subscribe email)" => "여러 이메일을 한 번에 구독 가능 (tiki_p_subscribe email 필요)",
...
"Exercise" => "연습",
"Creates exercises within a wiki page for students to test their new skills. Inline questions and optional answers selections can be defined. A score is given for multiple exercises within a page" => "학생들에게 새 능력을 시험하기 위하여 위키 페이지 내부에 연습문제를 만듭니다. 플러그인은 선택이 되어질 인라인 질문과 선택적 답안을 정의하는 것을 허용합니다. 점수는 페이지 내부의 여러 개의 연습문제에 대하여 주어짐",
"Answer" => "답",
...
"Toggle Tabs" => "탭 토글",
"Allow toggling from tabs to no tabs view" => "탭에서 탭 없음 보기로 토글하는 것을 허용",
...
"Copyright allows a copyright to be determined for all the objects of tiki" => "저작권 기능은 티키의 모든 개체에 대한 저작권을 결정하는 것을 허용합니다",
...
"txt file, one email per line" => "txt 파일, 한 줄 당 한 개의 이메일",
